...
unsigned short random_schick(short val) {
	...
	if (val == 0)
		return 0;
/*
	ax = readw(0x4ba0);		// get rand_seed 
	ax = ax ^ readw(0xc3bf);	// XOR with rand_seed2 /
	ax = my_rol16(ax, 2);		// ROL ax /
	ax = ax + readw(0xc3bf);	// ADD rand_seed2 /
	ax = ax ^ readw(0x4ba0);	// XOR with rand_seed /
	ax = my_rol16(ax, 3);
	bx = ax;
	dx = (ax < 0) ? -1 : 0;		// emulate CWD /
	ax = (ax ^ dx) - dx + 1 ;
	writew(0x4ba0, ax);		// update rand_seed /
	ax = bx;
	dx = (ax < 0) ? -1 : 0;		// emulate CWD /
	ax = (ax ^ dx) - dx;
	dx = (ax < 0) ? -1 : 0;		// emulate CWD /
...
unsigned short is_in_byte_array(char val, Bit8u *p) {
	...
	for (i = 1; (signed char)readb(p) != -1; i++) {
		p_tmp = p;
		p += 1;
		if (readb(p_tmp) == val)
			return i;
