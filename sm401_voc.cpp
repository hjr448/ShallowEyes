...
int CVoc::LoadVocBuffer(unsigned char *szBuffer, VOCID *pVocID)
{
	int wr = -14;
	LPVOCFILEINFO pVocInfo;
	LPVOCFILEHEADER	pVocFileHeader;
	pVocInfo = new VOCFILEINFO;
	if (pVocInfo){
		pVocFileHeader = (LPVOCFILEHEADER)szBuffer;

		if (!_strnicmp(pVocFileHeader->s1, "Creative Voice File", 13) )
		{
			pVocInfo->ulDataSize = pVocFileHeader->size+256*pVocFileHeader->siz-3;
			pVocInfo->ulDataOffset = 0x28;
			pVocInfo->sample_r=256000000/(65536-pVocFileHeader->time_const);
printf("pVocInfo: %x, %x, %x\n",pVocInfo->ulDataSize,pVocInfo->ulDataOffset,pVocInfo->sample_r);
			wr = 0;
		}
		if (wr==0){
			...
			if (pVocInfo->pData){
				// Read Sample Data
				memcpy (pVocInfo->pData, szBuffer+pVocInfo->ulDataOffset, pVocInfo->ulDataSize);
				long lLoop = 0;
				for (lLoop = 0; lLoop < MAX_NUM_VOCID; lLoop++){
					if (!m_VocIDs[lLoop]){
